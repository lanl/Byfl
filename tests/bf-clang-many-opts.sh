...
"$clang" -O2 -g -o simple-many-opts "$srcdir/simple.c"
...
env BF_DISABLE=byfl \
  "$PERL" -I"$top_srcdir/tools/wrappers" \
    "$bf_clang" -bf-plugin="$top_builddir/lib/bytesflops/.libs/bytesflops.so" \
                -bf-verbose -O2 -g -o simple-many-opts "$srcdir/simple.c" \
...
int_ops=`"$top_builddir/tools/postproc/bfbin2csv" --include=Program --flat-output simple-many-opts.byfl | "$AWK" -F, '$3 ~ /Integer operations/ {print $4}'`
