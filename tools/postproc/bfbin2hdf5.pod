...
Hdf5 database and then deleted, one can save time and disk space by
writing Byfl binary output to a named pipe and running
B<bfbin2hdf5> on that named pipe:
...
bfbin2text(1), bfbin2sqlite3(1), bf-gcc(1), bf-g++(1), bf-gfortran(1),
