...
bfbin2xlxml - convert Byfl output to XML Spreadsheet format

=head1 SYNOPSIS

B<bfbin2xlxml> I<input_file.byfl> [I<output_file.xml>]

=head1 DESCRIPTION

By default, applications instrumented with Byfl write measurement data
to a binary F<.byfl> file.  B<bfbin2xlxml> converts such files to
...
B<bfbin2xlxml> has no command-line options.  The program accepts the
...
writing Byfl binary output to a named pipe and running B<bfbin2xlxml>
on that named pipe:

    $ mkfifo myprog.pipe
    $ bfbin2xlxml myprog.pipe myprog.xml &
