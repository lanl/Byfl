...
The I<callback_list_len> argument specifies the caller's view of the
number of bytes in a I<bfbin_callback_t> and is used as a crude form
of version checking (i.e., to ensure that a client program was not
built against one version of the F<bfbin> library and dynamically
linked against a different version).  Finally, the I<user_data>
argument is an arbitrary pointer that will be passed to each callback
function.  Client programs will typically point I<user_data> to a
structure of local state information.
...
bfbin2hdf5(1), bfbin2csv(1), bfbin2sqlite3(1), bf-gcc(1), bf-g++(1),
