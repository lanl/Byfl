...
namespace bytesflops_pass {
  ...
  void BytesFlops::track_global_variables (Module* module) {
    ...
    for (unsigned c = 0; c < nmd->getNumOperands(); c++) {
      ...
      DIDescriptor cu_node(nmd->getOperand(c));
      if (!cu_node.isCompileUnit())
        continue;
      DICompileUnit cu(cu_node);
      DIArray gvs = cu.getGlobalVariables();
      for (unsigned int g = 0; g < gvs.getNumElements(); g++) {
        // For each global variable in the current compile unit...
        DIDescriptor gv_desc = gvs.getElement(g);
        if (!gv_desc.isGlobalVariable())
          continue;   // Not a global variable

        // Determine various properties of the global variable.
        DIGlobalVariable gv(gv_desc);
        StringRef gv_name = gv.getLinkageName();
        if (gv_name == "")
          gv_name = gv.getDisplayName();
        Constant* gv_const = gv.getConstant();
        if (gv_const == nullptr)
